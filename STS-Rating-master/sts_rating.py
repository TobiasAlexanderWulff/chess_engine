...
def analyze_pos(
            ...
            for line in inFO:
                ...
                for eline in iter(p.stdout.readline, ""):
                    ...
                    if proto:
                        ...
                        if (
                            "move" in eline
                            and "increment" not in eline
                            and "moves_left" not in eline
                            and "time" not in eline
                            and "TOURNAMENT" not in eline
                            and "white" not in eline
                            and "black" not in eline
                            and "#" not in eline
                        ):
                            a = eline.strip()
                            a = a.split(" ")
                                ...
                                if bm == item:
                                    ...
                                    if i == 0:
                                        ...
                                        if debug:
                                            ...
                                            logfnFO.write(
                                                "Position points earned             : %d\n"
                                                % (scoreList[i])
                                            )
                                            logfnFO.write(
                                                "Total points for this theme so far : %d/%d\n\n"
    ...
    with open(resultFN, "a") as resFO:
        ...
        if proto == 0:  # UCI
            resFO.write(
                "Hash: %d, Threads: %d, time/pos: %0.3fs\n\n"
        ...
        resFO.write(
            "%8s %6s %6s %6s %6s %6s %6s %6s %6s %6s %6s %6s %6s %6s %6s %6s %6s\n"
            % (
                "STS ID",
                "STS1",
                "STS2",
        ...
        resFO.write(
            "%8s %6d %6d %6d %6d %6d %6d %6d %6d %6d %6d %6d %6d %6d %6d %6d %6d\n"
            % (
                "NumPos",
                r[0][1],
                r[1][1],
        ...
        if maxScore:
            resFO.write(
                "%8s %6.1f %6.1f %6.1f %6.1f %6.1f %6.1f %6.1f %6.1f %6.1f %6.1f %6.1f %6.1f %6.1f %6.1f %6.1f %6.1f\n"
                % (
                    "Score(%)",
                    r[0][3],
                    r[1][3],
        ...
        if bCalculateRating:
            ...
            Rating = slope * sp + intercept
            resFO.write(
                "%8s %6.0f %6.0f %6.0f %6.0f %6.0f %6.0f %6.0f %6.0f %6.0f %6.0f %6.0f %6.0f %6.0f %6.0f %6.0f %6.0f\n"
                % (
                    "Rating",
                    slope * r[0][3] + intercept,
                    slope * r[1][3] + intercept,
                    slope * r[2][3] + intercept,
                    slope * r[3][3] + intercept,
                    slope * r[4][3] + intercept,
                    slope * r[5][3] + intercept,
                    slope * r[6][3] + intercept,
                    slope * r[7][3] + intercept,
                    slope * r[8][3] + intercept,
                    slope * r[9][3] + intercept,
                    slope * r[10][3] + intercept,
                    slope * r[11][3] + intercept,
                    slope * r[12][3] + intercept,
                    slope * r[13][3] + intercept,
                    slope * r[14][3] + intercept,
                    Rating,
                )
            )
        ...
        for (
            i,
            n,
        ) in enumerate(ResultDataByScorePercent):
            idNum = n[0].split("v")
            idNum = idNum[1].split(".")
            idNum = int(idNum[0])
            resFO.write(
...
def main(argv):
    ...
    if sEngine is None:
        ...
        if protocol == 0:
            print(
                "Hash: %d, Threads: %d, MoveTime: %0.1fs"
                % (nHash, nThreads, float(nMoveTime) / 1000)
            )
